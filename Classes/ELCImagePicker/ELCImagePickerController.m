...
- (BOOL)shouldSelectAsset:(ELCAsset *)asset previousCount:(NSUInteger)previousCount
{
    ...
    if (!shouldSelect) {
        if ([_imagePickerDelegate respondsToSelector:@selector(elcImagePickerControllerDidSelectTooManyAssets:)]) {
            [_imagePickerDelegate performSelector:@selector(elcImagePickerControllerDidSelectTooManyAssets:) withObject:self];
        }
        
        NSString *title = [NSString stringWithFormat:NSLocalizedString(@"Only %d photos please!", nil), self.maximumImagesCount];
        NSString *message = [NSString stringWithFormat:NSLocalizedString(@"You can only upload %d photos at a time.", nil), self.maximumImagesCount];
